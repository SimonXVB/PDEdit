...
export function UploadButton({ handleFile }: { handleFile: (e: React.ChangeEvent<HTMLInputElement>) => void }) {
    ...
    return (
        <>
            <button onClick={() => inputButtonRef.current!.click()} className="flex flex-col items-center relative py-8 px-24 bg-cyan-500 text-white rounded-xl font-semibold cursor-pointer hover:bg-cyan-500/80 hover:-translate-y-0.5">
                ...
                <div><strong>Choose a PDF</strong></div>
            </button>
            <input type="file" ref={inputButtonRef} onChange={handleFile} className="hidden"/>
        </>
    )
};