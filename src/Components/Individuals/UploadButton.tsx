...
export function UploadButton({ handleFile }: { handleFile: (e: Event) => void }) {
    const inputButtonRef = useRef<HTMLInputElement>(null);

    useEffect(() => {
        const ref = inputButtonRef.current!;

        ref.addEventListener("change", (e) => handleFile(e));
        return ref.removeEventListener("change", handleFile);
    ...
    return (
        <button onClick={() => inputButtonRef.current!.click()} className="flex flex-col items-center relative p-8 bg-cyan-500 text-white rounded-xl font-semibold cursor-pointer hover:bg-cyan-500/80 hover:-translate-y-0.5">
            ...
            <input type="file" ref={inputButtonRef} className="absolute w-full top-0 h-full opacity-0 cursor-pointer select-none"/>
        </button>
    )
};