...
export function UploadButton({ handleFile }: { handleFile: (e: React.ChangeEvent<HTMLInputElement>) => void }) {
    ...
    return (
        <button className="button" onClick={() => inputButtonRef.current!.click()}>
            ...
            <span className="front">
                <div className="flex flex-col items-center p-5">
                    ...
                    <input type="file" onChange={handleFile} ref={inputButtonRef} className="absolute w-full h-full opacity-0 cursor-pointer"/>
                </div>
            </span>
        </button>
    )
